...
import { statusMenuList} from "./left-menu/menu";
...
export default {
  ...
  methods: {
    ...
    dialogOpen() {
      const menuList = JSON.parse(JSON.stringify(statusMenuList))
      if (this.currentOperateItem.name) {
        menuList[0].componentList = menuList[0].componentList.filter(v => v.type === 'status-operate-operate')
      } else {
        menuList[0].componentList = menuList[0].componentList.filter(v => v.type !== 'status-operate-operate')
      }
      this.mList = menuList;
    ...
    initStatusFlow() {
      ...
      this.$nextTick(()=>{
        ...
        var height = 700;
        _this.nextflow = new StatusFlow(width, height);
    ...
    deleteDiagram: function () {
      ...
      if (r == true) {
        if (this.selectedDiagram !== undefined) {
          ...
          this.currentOperateItem._statusOperateSetTextOfType = ''
        }
      }
    },
