...
export default {
  ...
  methods: {
    ...
    checkDataInputType(componentDataInputType, level, dataSourceType, dataMap) {
      ...
      console.log('componentDataInputType====', componentDataInputType);
      if (componentDataInputType === "Array") {
        console.log('resData===', resData);
        console.log('resData1===', resData === "object");
    ...
    selectParentId(value) {
      ...
      if (tempValue && name &&　id && parentId　) {
        ...
        this.$set(this.componentConfig.options,'treeData',newDealData)
      }
    },
    // 处理数据
    dealData(arr, setOptions) {
      ...
      let newData = []
      arr.forEach(item => {
        item.title = item[setOptions.name].toString()
