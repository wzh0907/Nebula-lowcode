...
import formConfigModule from "@/views/report/layout/report-config-basics/basics-config/form-config";
import {getStorage,setStorage} from "@/util/storageUtil.js";
export default {
  name: "data-source-api-config",
  props: [
    "componentList",
    "componentConfig",
    "interfaceDataConfig",
  ...
  watch: {
    componentConfig: {
      ...
      handler(val) {
        ...
        // this.componentConfig.configList && this.componentConfig.configList.forEach(item => {
        //   if (item.key === 'contentDataSource') { // 需要优化，这里用key作为依据，并不可靠
        //     this.typeConfigs = item.typeConfigs || []
        //   }
        // })
      },
    },
  ...
  created() {
    // this.interfaceDataConfig = this.componentConfig.options.interfaceDataConfig || {}
    this.initData();
  ...
  mounted() {
    if (this.interfaceDataConfig.id) {
      this.initApi(true);
      // this.handleTypeConfig(this.componentConfig.options[dataSourceConfigKey])
    }
    this.handleTypeConfig(
      this.componentConfig.options[this.dataSourceConfigKey]
    );
    // let tempArr = []
  ...
  methods: {
    ...
    handleVarDataList() {
      // let interfaceArr = []
      // let componentArr = []
      // for(let key in this.$globalData.modelDataPool.interface) {
      //   interfaceArr.push({
      //     label: key,
      //     value: this.$globalData.modelDataPool.interface[key]
      //   })
      // }
      // for(let key in this.$globalData.modelDataPool.component) {
      //   let value = ''
      ...
      //     label: '接口',
      //     value: 'interface',
      //     children: interfaceArr
      //   }, {
      //     label: '组件',
      //     value: 'component',
      //     children: componentArr
      //   }
      // ]
      // 获取本地存储的配置属性
    ...
    chooseInterfaceDataKey(key) {
      ...
      if (listTemp && typeof listTemp.push === "function") {
        // this.componentConfig.type === 'van-design-list' && saveAndUpdateLocalData(this.componentConfig.uuid, listTemp || [])
        this.componentConfig.type === "van-design-list" &&
    ...
    queryDataTableFieldV4(tableName, isNoClear) {
      !isNoClear && this.clearChartSettings();
      this.interfaceDataConfig.tableName = tableName;
      this.tablesListApi.forEach((tableItem) => {
        // 保存数据集名称
      ...
      getTableFieldService({ dbConfigId, tableName }).then((res) => {
        this.tableFieldList = res.dataList;
        // this.pickChartData('', this.tableFieldList) // TODO:::test 待处理
      });
      // 条件查询数据表数据
    ...
    queryOpenDataTableResult() {
      ...
      getOpenDataTableResult(params).then((res) => {
        ...
        // 保存到本地缓存，做数据绑定使用
        // this.componentConfig.type === 'van-design-list' && saveAndUpdateLocalData(this.componentConfig.uuid, res.dataList || [])
        this.componentConfig.type === "van-design-list" &&
          this.$pageDataModelEngine.saveCustomData(
            this.componentConfig.uuid,
            res.dataList || []
      ...
      getProcesseDataList(data).then((res) => {
        if (res.code === 0) {
          ...
          if (this.componentConfig.type === "van-design-progress") {
            this.interfaceDataConfig.interFaceData = res.data; // 进度条后端服务
          }
          ...
          // MVC改造
          // this.$globalData.modelDataPool.interface['getProcesseDataList: dataApiId: '+ id] = res.data
        } else {
...
<template>
  <!-- 方块单选 -->
  <el-radio-group
    v-model="componentConfig.options[dataSourceConfigKey]"
    size="small"
  ...
  >
    ...
    <div
      ...
      v-if="
        componentConfig.options[dataSourceConfigKey] === 'radio-button-default'
      "
    >
    ...
    <div
      style="padding-top: 10px"
      v-if="
        componentConfig.options[dataSourceConfigKey] ===
        'radio-button-interface'
      "
    >
      <el-select
      ...
      <el-select
        ...
        v-if="componentConfig.type !== 'van-design-progress'"
        placeholder="请选择数据"
        style="padding-top: 10px"
    ...
    <div
      style="padding-top: 10px"
      v-if="
        componentConfig.options[dataSourceConfigKey] === 'radio-button-dataset'
      "
    >
      <el-select
    ...
    <div
      style="padding-top: 10px"
      v-if="
        componentConfig.options[dataSourceConfigKey] === 'radio-button-tables'
      "
    >
      <el-select
    ...
    <div
      style="padding-top: 10px"
      v-if="
        componentConfig.options[dataSourceConfigKey] === 'radio-button-bind'
      "
    >
    ...
    <div
      v-if="
        [
          ...
          'radio-button-dataset',
          'radio-button-interface',
        ].indexOf(componentConfig.options[dataSourceConfigKey]) !== -1 &&
    ...
    <div
      v-if="
        componentConfig.options[dataSourceConfigKey] ===
          'radio-button-tables' && componentConfig.type === 'van-design-form'
      "
    >
      <el-form
        size="small"
        ...
        class="label-left"
      >
        <el-form-item label="输入项配置"> </el-form-item>
        <template
        ...
        >
          <formConfigModule
            ...
            :dataTableFieldList="tableFieldList"
