<template>
  <div class="selectTypesOpert">
    <!-- 默认手输 -->
     <div v-if="componentConfig.options.contentDataSource === 'radio-button-default'">
      <!-- 数值输入按钮 -->
    ...
    <template v-else >
      <el-form-item>
        <span>最大值:</span>
        <el-select v-model="componentConfig.options.interfaceDataConfig.maxKey" placeholder="请选择最大值" style="padding-top: 10px;" key="maxValue" @change="chooseMaxValue">
          <el-option
      ...
      <!-- <el-input type="number" disabled placeholder="请输入最大值" v-model.number="componentConfig.options.interfaceDataConfig.maxValue" key="maxValue" style="padding-top: 10px;">
          ...
          <el-form-item>
            <span>进度值:</span>
            <el-select v-model="componentConfig.options.interfaceDataConfig.rateKey" placeholder="请选择进度值" style="padding-top: 10px;" key="value" @change="chooseRateValue">
              <el-option
                v-for="(value, key) in componentConfig.options.interfaceDataConfig.interFaceData"
                :key="key"
                :label="key"
...
export default {
  ...
  methods: {
    chooseMaxValue(key){
       let currentValue = this.componentConfig.options.interfaceDataConfig.interFaceData[key];
      this.componentConfig.options.interfaceDataConfig.maxValue=Number(currentValue);
    },
    chooseRateValue(key){
      let rateValue = this.componentConfig.options.interfaceDataConfig.interFaceData[key];
      this.componentConfig.options.interfaceDataConfig.rateValue=Number(rateValue);
    }
  },
};
</script>
