    ...
    <div class="split-pane-page-wrapper right-menu-div">
      <div slot="left" class="pane left-pane">
        <div v-if="drawer">
          <LeftMenu
            ...
            @snippetHandler="v => $refs['CodeEditorEvent'].snippetHandler(v)"
          />
        </div>
        <div v-else id="drag-items" class="shapes">
          <el-collapse
      ...
      >
        ...
        <CodeEditorEvent
          ref="CodeEditorEvent"
          :curDiagram="curDiagram"
          v-if="curDiagram && curDiagram.properties && curDiagram.properties.actionType === 'event-operate-javascript'"
        />
      </div>
      <!-- <el-drawer
        :title="drawerTitle"
...
import LeftMenu from "./right-menu/event-item.vue";
import CodeEditorEvent from './components/code-editor-event.vue'
...
import { getReportTreeListV5Service,getProjectFileTreeListV5Service } from "@/api/v5";
export default {
  name: "addRole",
  props: {
    id: {
  ...
  methods: {
    ...
    dialogOpen() {
      if (this.deviceDesignType == "PC") {
        this.mList = pcMenuList;
      } else if(this.deviceDesignType == "workFlow") {
        this.mList = workFlowList;
        this.deviceDesignType="PC"
      }else {
        this.mList = mobileMenuList;
      }
      this.drawer=false
      this.initEventFlow();
      // 初始化 接口，页面以及流程中使用的接口（返回值）
      this.handleEventInterface();
    ...
    handleEventInterface(){
      this.eventList.forEach(eventItem=>{
        if(eventItem.type=='event-operate-interface'){
          let currentActionItem=eventItem.properties
          this.queryProcessApiParamAndRes(currentActionItem.operateApiId,currentActionItem);
      ...
      getProcessParamService(data).then((res) => {
        ...
        paramsList &&
          paramsList.forEach((item) => {
            ...
            currentActionItem.interfaceDataConfig.paramsConfigs.forEach(pItem=>{
    ...
    initEventFlow() {
      ...
      this.$nextTick(()=>{
        var a = Konva;
        var width = document.body.clientWidth * 0.6 - 260 - 10;
        var height = 700;
        ...
        if (_this.eventList.length > 0) {
          nextflow.destroyChildren();
          let scheme = {
            nodes: _this.eventList,
          };
          nextflow.import(scheme);
        } else {
          // this.initData();
          nextflow.import({
            nodes: [
    ...
    onSelect(diagram) {
      ...
      this.drawerTitle = diagram.title;
      this.$nextTick((item) => {
        _this.$refs["leftMenu"].init(diagram);
