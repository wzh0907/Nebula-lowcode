...
export default {
  ...
  data() {
    return {
      activeNames: [0],
      interfaceDataAll: [], // 接口数据请求到的数据
      reportList: [], // 预览下拉列表
      dataListID: "",
      interfaceKeyList: [], // 值key
  ...
  created() {
    if (
      this.styleEditorConfig.textHtmlPosit !== undefined ||
      this.styleEditorConfig.textHtmlPosit !== null
    ) {
    ...
    if (
      this.styleEditorConfig.textHtmlBlod !== undefined ||
      this.styleEditorConfig.textHtmlBlod !== null
    ) {
    ...
    if (
      this.styleEditorConfig.textHtmlTilt !== undefined ||
      this.styleEditorConfig.textHtmlTilt !== null
    ) {
    ...
    if (
      this.styleEditorConfig.textHtmlUnder !== undefined ||
      this.styleEditorConfig.textHtmlUnder !== null
    ) {
  ...
  methods: {
    ...
    styleButtonSelected(style, index) {
...
<template>
  <div class="textHtmlStyle">
    <ul>
        ...
        <button
          ...
          :class="{
            'selected-style-button':
              item.fontSize == styleEditorConfig.textHtmlSize &&
              item.fontWeight == styleEditorConfig.textHtmlBlod &&
              item.color == styleEditorConfig.textColor,
          }"
        >
          {{ textStyleName(index) }}
        </button>
      ...
      <li>
        <el-select
          v-model="styleEditorConfig.textHtmlSize"
          style="width: 145px"
          @change="selsetChangeStyle($event, 'size')"
        >
          <el-option
      ...
      <li style="margin-top: -7px; padding: 0px">
        <div
          class="colorSetting"
        ...
        >
          <span class="colorStyle">
            ...
            <colorPicker
              :key="1"
              v-model="styleEditorConfig.textBgColor"
        ...
        >
          <span>
            <i
              ...
              :style="{ color: styleEditorConfig.textColor }"
            />
            <colorPicker
              :key="2"
              v-model="styleEditorConfig.textColor"
              style="visibility: hidden"
    ...
    <ul style="overflow: hidden; width: 100%">
      <li>
        <i
          class="iconfont iconzuoduiqi1"
          :class="activeLeftPosit ? 'active' : ''"
          @click.prevent="selsetChangeStyle($event, 'left')"
        />
      </li>
      <li>
        <i
          class="iconfont iconjuzhongduiqi"
          :class="activeCenterPosit ? 'active' : ''"
          @click.prevent="selsetChangeStyle($event, 'center')"
        />
      </li>
      <li>
        <i
          class="iconfont iconyouduiqi1"
          :class="activeRightPosit ? 'active' : ''"
          @click.prevent="selsetChangeStyle($event, 'right')"
        />
      </li>
      <li>
        <i
          class="iconfont iconzitijiacu"
          :class="activeTextHtmlBlod ? 'activeBlod' : ''"
          @click.prevent="selsetChangeStyle($event, 'blod')"
        />
      </li>
      <li>
        <i
          class="iconfont iconzitixieti"
          :class="activeTextHtmlTilt ? 'activeTilt' : ''"
          @click.prevent="selsetChangeStyle($event, 'xieti')"
        />
      </li>
      <li>
        <i
          class="iconfont iconzitixiahuaxian"
          :class="activeTextHtmlUnder ? 'activeUnder' : ''"
          @click.prevent="selsetChangeStyle($event, 'under')"
        />
      </li>
    </ul>
  </div>
