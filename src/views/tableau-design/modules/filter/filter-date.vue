...
// 引入类型
import { tableFieldServiceType } from '../../types/service-type'
import { filterConfigItemInterface } from '../../types/type'
...
export default class DateFilter extends Vue {
  ...
  dialogOpen() {
    ...
    let dateConfig: any = this.selectedFilterData || {} as any;
    if (!dateConfig || this.isEmptyObject(dateConfig)) return false

    // this.selectedType = dateConfig.type;
    this.startDate = dateConfig.startDate;
    this.endDate = dateConfig.endDate;
    let config: any = dateConfig.dateConfig;
