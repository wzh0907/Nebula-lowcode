<template>
  <div class="group-wrap">
    <div class="group-content">
      <p class="group-title">
        通知组件可以构建自定义执行日志或通知消息，并发送到邮件、短信、钉钉和企业微信。这些通知消息可以是静态消息，也可以是使用输入参数生成的动态消息。
      </p>
      <Form class="command-form" :model="formItem" :label-width="80" label-position="left">
        ...
        <FormItem label="接受者">
          <Input :value="getValueStr()" placeholder="请输入.."  @on-focus="setValue()" icon="ios-search-outline" readonly></Input>
        </FormItem>
        <FormItem label="标题" >
...
<script>
  export default {
    ...
    methods: {
      ...
      getValueStr(){
        return typeof this.account == 'object' ? `${this.valueMap[this.account.value_type]} ${this.account.value_type=="Static"?(this.account.data_value||''):(this.account.property_name||'')}` : this.account
      },
      setValue () {
        this.modal1 = true
        this.opType = 3
