...
import {setStorage} from "@/util/storageUtil.js";
/* eslint-disable */
function Diagram(nextflow, id, stage, layer, devSwitch) {
    ...
    this.chosenDiagramList = [];
}



const ArrowDefaultXOffset = 200;
const ArrowDefaultYOffset = 34;
...
const DiagramHeight = 78;

Diagram.prototype.create = function (type, imgfile, title, x, y, output_count, onSelected) {
    this._createShapeImage(type, undefined, imgfile, title, x, y, output_count, onSelected);

}
Diagram.prototype.createConnector = function (type, subType, imgfile, title, x, y, output_count, onSelected) {
    this._createShapeImage(type, subType, imgfile, title, x, y, output_count, onSelected);

}

Diagram.prototype.createNewConnector = function (type, subType, imgfile, title, x, y, output_count, onSelected) {
    this._createNewShapeImage(type, subType, imgfile, title, x, y, output_count, onSelected);
}


Diagram.prototype._createShape = function (type, subType, img, title, properties, x, y, onSelected) {
    ...
    //     strokeEnabled: false
    // });
    let menuInfo = getMenuInfo(subType || type);
    ...
    shape.on('dragmove', function (e) {
        ...
        for (var i = 0; i < this.diagram.inputs.length; i++) {
            ...
            if (in_arrow._m_isLinked) {
                __set_arrow_position(in_arrow, this.x() - 10, this.y() + 34);
                const points = __get_connector_points(
...
Diagram.prototype.createChosenCircle = function (x, y, diagram) {
    console.log("createChosenCircle", diagram)
    // 计算高度
    let children = diagram.shape.children;
    let chosenHeight = 0
    children && children.forEach(child => {
        if (child.className == 'Rect') {
            chosenHeight += child.height();
        }
    })
    const layer = this.layer;
    const stage = this.stage;
...
Diagram.prototype._createShapeImage = function (type, subType, imgfile, title, x, y, output_count, onSelected) {
    const stage = this.stage;
    const layer = this.layer;
    ...
    this.title = title;
    img.onload = function () {
        _this._createShape(type, subType, img, title, null, x, y, onSelected);
        _this._createOutputs(x, y, output_count);

    };
} //_createShape

Diagram.prototype._createNewShapeImage = function (type, subType, imgfile, title, x, y, output_count, onSelected) {
    const stage = this.stage;
    const layer = this.layer;
...
Diagram.prototype._importShape = function (type, subType, imgfile, title, properties, x, y, outputs, onSelected, onComplete) {
    const stage = this.stage;
    const layer = this.layer;
    ...
    this.title = title;
    img.onload = function () {
        _this._createShape(type, subType, img, title, properties, x, y, onSelected);
        for (var i = 0; i < outputs.length; i++) {
            var output = outputs[i];
            var isNeedDiyArrow = false;
            if (type == "route") {
...
Diagram.prototype._createOutputs = function (x, y, outputCount) {
    if (outputCount > 0) {
        ...
        for (var i = 0; i < outputCount; i++) {
            ...
            } else if (this.type === 'if' || this.type === 'ifV4') {
                if (i == 0) {
                    arrowTitle = 'True';
                } else {
                    arrowTitle = 'False';
    ...
    if (port_count == this.outputs.length) {
        if(this.type==='try'){
            ...
            for (var i = 0; i < port_count; i++) {
                var title = "";
                if(i<this.properties.exceptions.length){
                    title=this.properties.exceptions[i].name
                    this._setOutputTitle(i+2,title);
                }else if(i==this.properties.exceptions.length){
                    title='无异常'
                }else{
                    title='其他异常'
                }
    ...
    if (this.outputs.length > port_count) {
        ...
        for (var i = len; i < port_count; i++) {
            ...
            if (this.type === 'branch' || this.type === 'branchV4') {
                ...
                if(i<this.properties.exceptions.length+2){
                    title=this.properties.exceptions[i-2].name
                }else if(i==this.properties.exceptions.length+2){
                    title='无异常'
                }else{
                    title='其他异常'
                }
...
NextFlow.prototype.init = function () {
    ...
    stage.on('dragend', function (e) {
        ...
        var shape = layer.getIntersection(pos);
        if (shape) {
            let group = shape.parent
            if (group && group._m_isShape) {
                previousShape.fire(
                    'drop', {
                    type: 'drop',
                    target: previousShape,
    ...
    stage.on('dragenter', function (e) {
        console.log('dragenter', _this.draggingShape, e.target.id, e, e.target, this);
        if (e.target.parent._m_isShape && _this.draggingShape._m_isArrow && _this.draggingShape.shape._id !== e.target.parent._id && e.target.parent.diagram.type !== 'startV5') {
            // e.target.fill('#ccc');
    ...
    stage.on('drop', function (e) {
        ...
        if (target._m_isShape && source._m_isArrow && source.shape) {
            ...
            if (source.shape._id === target._id || target.diagram.type === 'startV5') {
                ...
                __set_arrow_position(source, target.x() - 10, target.y() + 34);
                const points = __get_connector_points(
                    source.shape.position(),
                    source.position()
                );
...
NextFlow.prototype._createDiagram = function (type, title, x, y, outputs) {
    ...
    var imgfile = require('@/assets/icon/' + type + '.svg');
    var _this = this;
...
NextFlow.prototype._createDiagramV4 = function (type, subType, title, x, y, outputs) {
    ...
    // var imgfile = require('@/assets/icon/'+type+'.jpg');
    var imgfile = require('@/assets/icon/' + subType + '.svg');
    var _this = this;
...
NextFlow.prototype._createStart = function (title, x, y) {
    var diagram = this._createDiagram('startV4', title, x, y, 1);
...
NextFlow.prototype._createTry = function (title, x, y) {
    var diagram = this._createDiagram('try', title, x, y, 2);
...
NextFlow.prototype._createDataProcessFunction = function (title, subType, x, y) {
    var diagram = this._createDiagramV4('dataprocessfunction', subType, title, x, y, 1);
...
NextFlow.prototype._createAssociationV4 = function (title, x, y) {
    var diagram = this._createDiagram('associationV4', title, x, y, 1);
    return diagram;
}
...
NextFlow.prototype._createWorkFlow = function (type,title, x, y) {
    var diagram = this._createDiagram(type, title, x, y, 1);
    return diagram;
}


NextFlow.prototype.createDiagram = function (type, x, y) {
    if (type === 'startV4') {
        this._createStart('开始', x, y);
    } else if (type === 'startV5') {
        this._createStart('开始', x, y);
    } else if (type === 'stopV4') {
        this._createStop('结束', x, y);
    } else if (type === 'loopV4') {
        this._createLoop('循环', x, y);
    } else if (type === 'connectorV4') {
        this.createConnector('数据', x, y);
    } else if (type === 'connector-query') {
        this.createConnector('数据装载', type, x, y);
    } else if (type === 'connector-insert') {
        this.createConnector('数据新增', type, x, y);
    } else if (type === 'connector-update') {
        this.createConnector('数据修改', type, x, y);
    } else if (type === 'connector-delete') {
        this.createConnector('数据删除', type, x, y);
    } else if (type === 'connector-custom') {
        this.createConnector('自定义', type, x, y);
    } else if (type === 'connectorV3') {
        this.createConnectorV3('数据V3', x, y);
    } else if (type === 'branchV4') {
        this._createBranch('分支', x, y, 2);
    } else if (type === 'routeV4') {
        this._createRoute('路由', x, y, 1);
    } else if (type === 'ifV4') {
        this._createIf('条件', x, y);
    }  else if (type === 'try') {
        this._createTry('异常处理', x, y);
    } else if (type === 'dataprocessV4') {
        this._createDataProcess('函数', x, y);
    } else if (type === 'dataprocess-number') {
        this._createDataProcessV4('数值', type, x, y);
    } else if (type === 'dataprocess-string') {
        this._createDataProcessV4('文本', type, x, y);
    } else if (type === 'dataprocess-date') {
        this._createDataProcessV4('日期', type, x, y);
    } else if (type === 'dataprocess-list') {
        this._createDataProcessV4('列表', type, x, y);
    } else if (type === 'dataprocess-object') {
        this._createDataProcessV4('对象', type, x, y);
    } else if (type === 'dataprocess-other') {
        this._createDataProcessV4('赋值', type, x, y);
    } else if (type === 'dataprocess-map') {
        this._createDataProcessV4('字典', type, x, y);
    } else if (type === 'dataprocessfunction') {
        this._createDataProcessFunction('函数', type, x, y);
    } else if (type === 'setproperties') {
        this._createSetProperties('设置变量', x, y);
    } else if (type === 'programcommand') {
        this._createProgramCommand('命令', x, y);
    } else if (type === 'returnV4') {
        this._createReturn('输出', x, y);
    } else if (type === 'processcallV4') {
        this._createCall('流程', x, y);
    } else if (type === 'messageV4') {
        this._createMessage('消息', x, y);
    } else if (type === 'notify') {
        this._createNotify('通知组件', x, y);
    } else if (type === 'exception') {
        this._createException('异常组件', x, y);
    } else if (type === 'interface') {
        this._createInterface('接口组件', x, y);
    } else if (type === 'interfaceV4') {
        this._createInterfaceV4('接口组件', x, y);
    } else if (type === 'scriptV4') {
        this._createScriptV4('代码', x, y);
    } else if (type === 'log') {
        this._createLog('日志', x, y);
    } else if (type === 'dataJoinV4') {
        this._createDataJoinV4('数据集连接', x, y);
    } else if (type === 'dataUnionV4') {
        this._createDataUnionV4('数据集合并', x, y);
    } else if (type === 'dataSortV4') {
        this._createDataSortV4('数据集排序', x, y);
    } else if (type === 'associationV4') {
        this._createAssociationV4('数据集内嵌', x, y);
    }  else if (type === 'flowStart') {
        this._createWorkFlow(type,'发起审批', x, y);
    } else if (type === 'flowList') {
        this._createWorkFlow(type,'工作流列表', x, y);
    } else if (type === 'flowLog') {
        this._createWorkFlow(type,'审批记录', x, y);
    }else {
        alert('未知类型');
    }
}
...
NextFlow.prototype._importShape = function (id, type, subType, title, x, y, outputs, properties, onComplete) {
    ...
    let img = require('@/assets/icon/' + type + '.svg');
    if (subType) {
        img = require('@/assets/icon/' + subType + '.svg')
    }
    // diagram._importShape(type, require('@/assets/icon/' + type + '.jpg'), title,properties, x, y, outputs, onClick, onComplete);
    diagram._importShape(type, subType, img, title, properties, x, y, outputs, onClick, onComplete);
}
...
NextFlow.prototype._createChosenRect = function () {
    ...
    var startPos = null;
    var chosenRect = null;
    _this.chosenDiagramList = new Array();
    ...
    stage.on('mouseup touchend', function (e) {
        ...
        if (e.evt.button == 0) {
            if (this.mouseEnterFlag) {
                ...
                _this.chosenDiagramList = new Array();
                // 计算框选的内容
                var images = layer.find("Group");
                console.log("框选的 xxxxx==", images)
                for (var i = 0; i < images.length; i++) {
                    ...
                    if (
                        ...
                        _this.chosenDiagramList.push(images[i].diagram);
            ...
            console.log(_this.chosenDiagramList);
        }
    ...
    chosenGroup.on('dragmove', function (e) {
        _this.chosenDiagramList.forEach((diagram) => {
            // 移动图片和文字
            ...
            diagram.inputs.forEach((in_arrow) => {
                if (in_arrow._m_isLinked) {
                    __set_arrow_position(in_arrow, diagram.relative.x + e.target.x() - 10, diagram.relative.y + e.target.y() + 34);
                    const points = __get_connector_points(
                        in_arrow.shape.position(),
                        in_arrow.position()
                    );
...
NextFlow.prototype.importPasteData = function (jsondata, x, y) {
    var _this = this;
    _this.loadedCount = 0;
    ...
    let newList = [];
    // 这边要重新计算 output next 是谁，所以 需要做2次循环，第二次是创建
    for (var i = 0; i < data.length; i++) {
        var diagram = data[i];
        if (diagram.type != 'startV5') {
            var cdiagram = this._newDiagram();
            // var imgfile = require('@/assets/icon/'+diagram.type+'.jpg');
            var imgfile = require('@/assets/icon/' + diagram.type + '.svg');
            var _this = this;

            let pasteX = Number(x) - (firstX - diagram.x);
            let pasteY = Number(y) - (firstY - diagram.y);
            ...
            newList.push(cdiagram);
            cdiagram.createNewConnector(diagram.type, diagram.subType, imgfile, diagram.title, pasteX, pasteY, diagram.outputs.length, onClick);
        }
    }
    // newList.forEach((ndiagram)=>{
    //     var imgfile = require('@/assets/icon/' + ndiagram.type + '.svg');
    //     let nOutputs=[]
    //     ndiagram.outputs.forEach(oItem=>{
    //         let temp={
    //             id:oItem.id,
    //             x:oItem.x,
    //             y:oItem.y,
    ...
    //         }
    //         if(oItem.next){
    //             newList.forEach((tempdiagram)=>{
    //                 if(tempdiagram.oldId== oItem.next){
    //                     temp.next=tempdiagram.id
    //                 }
    //             });
    //         }
    //         nOutputs.push(temp)
    ...
    //         }
    //     }
    //     ndiagram.createNewConnector(ndiagram.type, ndiagram.subType, imgfile, ndiagram.title, ndiagram.x , ndiagram.y , nOutputs, onClick);
...
NextFlow.prototype.copyDiagram = function (diagram) {
    console.log("copyDiagram ", JSON.stringify(diagram));
    // delete this._diagrams[diagram.id];
    var cdiagram = this._newDiagram();
    // var imgfile = require('@/assets/icon/'+diagram.type+'.jpg');
    var imgfile = require('@/assets/icon/' + diagram.type + '.svg');
...
NextFlow.prototype.copyNewDiagram = function (diagramList) {
    let entityList = [];
    diagramList && diagramList.forEach((diagram) => {
        const type = diagram.type;
        const links = diagram.links;
    ...
    setStorage("clipboard_process_data", JSON.stringify(entityList));
...
function __get_connector_points(from, to) {
    var points = [];
    const fromx = from.x + DiagramWidth - 5;
...
function getMenuInfo(menuType) {
    ...
    if (menuType === 'startV5') {
        menuInfo.name = "开始"
        menuInfo.color = "#FF9944"
    }
    console.log("getMenuInfo", menu, menuType);
    menu.menuList.forEach(menuItem => {
        menuItem.componentList.forEach(componentItem => {
            if (componentItem.type == menuType) {
                menuInfo.name = componentItem.name
                menuInfo.color = componentItem.color
