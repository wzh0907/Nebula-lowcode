...
const ArrowDefaultXOffset = 150;
const ArrowDefaultYOffset = 16;
    ...
    shape.on('dragmove', function (e) {
        ...
        for (var i = 0; i < this.diagram.inputs.length; i++) {
            ...
            if (in_arrow._m_isLinked) {
                __set_arrow_position(in_arrow, this.x() - 8, this.y() + 16);
                const points = __get_connector_points(
...
NextFlow.prototype.init = function () {
    ...
    stage.on('dragenter', function (e) {
        ...
        if (e.target._m_isShape && _this.draggingShape._m_isArrow && _this.draggingShape.shape._id !== e.target._id && e.target.diagram.type !== 'startV4') {
            e.target.fill('#ccc');
    ...
    stage.on('drop', function (e) {
        ...
        if (target._m_isShape && source._m_isArrow && source.shape) {
            ...
            if (source.shape._id === target._id || target.diagram.type === 'startV4') {
                ...
                // source.x(target.x()-8)
                // source.y(target.y()+16)
                __set_arrow_position(source, target.x() - 8, target.y() + 16);
                const points = __get_connector_points(
                    source.shape.position(),
                    source.position()
                );
    ...
    chosenGroup.on('dragmove', function (e) {
        _this.chosenDiagramList.forEach((diagram) => {
            ...
            diagram.inputs.forEach((in_arrow) => {
                if (in_arrow._m_isLinked) {
                    __set_arrow_position(in_arrow, diagram.relative.x + e.target.x() - 8, diagram.relative.y + e.target.y() + 16);
                    const points = __get_connector_points(
                        in_arrow.shape.position(),
                        in_arrow.position()
                    );
...
NextFlow.prototype.importPasteData = function (jsondata, x, y) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        // cdiagram.type=diagram.type;
        // cdiagram.subType=diagram.subType;
        // cdiagram.title=diagram.title
        // cdiagram.x=pasteX;
        // cdiagram.y=pasteY;
        // cdiagram.oldId=diagram.id

        // cdiagram.outputs=diagram.outputs
        function onClick(e) {
            var call = _this.events.click;
            if (call) {
                console.log('click create', e, this);
