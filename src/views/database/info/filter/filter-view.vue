  ...
  <div class="filter-view">
    ...
    <div v-show="filterType === 'match'">
      ...
      <el-form size="mini" :inline="true" class="form-gap form-gap-bottom" v-for="(item, index) in filterDataList" :key="index">
        ...
        <el-form-item>
          <el-select placeholder="请选择字段" v-model="item.field" @change="onChangeField(item, dataTableInfoMap[item.tableId])">
            <el-option :label="value.name" :value="value.name" v-for="(value, key) in dataTableInfoMap[item.tableId]" :key="key"></el-option>
          </el-select>
        </el-form-item>
