<template>
  <div class="app-container">
    <div class="db-div">
      ...
      >
        <i class="connect-icon iconfont iconconnect"></i>
        <div>
          <span class="db-datasource">{{ dbItem.name }}</span>
          <div style="word-break: break-all">{{ dbItem.host }}</div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    queryList() {
      ...
      if (this.executorUrl) {
        axios
          .get(this.executorUrl + "/dbconfig/v1/connect/list", params)
          .then(function (res) {
    ...
    editItem(row) {
      ...
      this.chooseEditId = row.dbPrimaryId;
